...
$cache_status_file = $cache_dir . '/status_cache.json';
...
function clear_cache_dir($dir) {
    ...
    foreach ($files as $file) {
        ...
        // 排除保存状态的 status_cache.json 文件
        if (basename($todo) === 'status_cache.json') {
            continue;
        }
        if ($file->isDir()) {
            @rmdir($todo);
...
if ($is_logged_in && isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'diagnostic') {
        ...
        @file_put_contents($cache_status_file, json_encode(array(
...
if ($is_logged_in && file_exists($cache_status_file)) {
    $history_status = json_decode(file_get_contents($cache_status_file), true);
}
